  ...
  <div class="term-list">
    ...
    <div class="search-bar">
      ...
      <div class="action-buttons">
        <el-button v-if="userStore.isAdmin" @click="openCategoryManageDialog">
    ...
    <div class="category-filter-section">
      ...
      <div class="category-tags">
        ...
        >
          {{ cat.icon }} {{ cat.name }}
          <span v-if="cat.id !== 'favorites'" class="tag-count">({{ categoryCountMap[cat.id]|| 0  }})</span>
    ...
    <el-table :data="tableData" border stripe style="margin-top: 24px" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="320" align="center">
        <template #default="scope">
          ...
          <el-button v-if="userStore.isAdmin" type="text" @click="openEditDialog(scope.row)">编辑</el-button>
          <el-button v-if="userStore.isAdmin" type="text" danger @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div v-if="selectedRows.length > 0" class="batch-action-bar">
      ...
      <el-button v-if="userStore.isAdmin" type="danger" size="small" @click="batchDelete">批量删除</el-button>
    </div>
    ...
    <el-dialog v-model="categoryManageVisible" title="管理分数据库" width="700px">
      <div class="category-manage-header">
        <el-button v-if="userStore.isAdmin" type="primary" size="small" @click="openAddCategoryDialog">
      ...
      <el-table :data="editableCategories" border stripe style="margin-top: 16px">
        ...
        <el-table-column label="术语数量" width="100" align="center">
          <template #default="scope">
            {{ categoryCountMap[scope.row.id] || 0  }}
          </template>
        </el-table-column>
        <el-table-column label="操作" width="200" align="center">
          <template #default="scope">
            <el-button v-if="userStore.isAdmin" type="text" size="small" @click="openEditCategoryDialog(scope.row)">编辑</el-button>
            <el-button
              ...
              :disabled="categoryCountMap[scope.row.id] || 0  > 0"
            >
              删除
            </el-button>
          </template>
...
const getTermPageList = async () => {
  try {
    ...
    if (selectedCategory.value === 'favorites') {
      tableData.value = tableData.value.filter(term => term.isLiked);
...
async function toggleFavorite(row) {
  if (row.isLiked) {
    await termApi.unfavorite(row.id);
  } else {
    await termApi.favorite(row.id);
  }
...
// 打开分数据库管理弹窗
const openCategoryManageDialog = async () => {
  await loadCategoryCount()
