...
import { userLogin, userRegister, checkUsername } from '@/api/user.js'
...
const doLogin = async () => {
  try {
    ...
    // res.data 是 {token: "...", user: {...}}
    userStore.login(res.data.token, res.data.user)
    console.log('当前角色:', res.data.user.roleId === 1 ? '管理员' : '普通用户')
    router.push('/home')
  } catch (error) {
    // 捕获接口错误信息（根据后端返回格式调整）
...
// 注册逻辑：先校验用户名是否存在，再调用注册接口
const doRegister = async () => {
  try {
    // 1. 先检查用户名是否存在
    const checkRes = await checkUsername(form.username)
    // 后端返回 Result.success(true/false)，data 字段为 boolean
