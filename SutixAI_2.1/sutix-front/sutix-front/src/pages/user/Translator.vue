  ...
  <div class="translator-page">
    ...
    <div v-if="isRestricted" class="restricted-overlay">
      <div class="restricted-card">
        ...
        <div class="restricted-title">翻译功能仅限管理员使用</div>
        <div class="restricted-desc">如需开通翻译权限，请联系管理员</div>
      </div>
    </div>

    <!-- 顶部功能栏 -->
    <div class="trans-header" :class="{ blurred: isRestricted }">
      ...
      <div class="switch-wrap">
        ...
        <el-button 
          ...
          size="small" 
          class="swap-btn"
          @click="isRestricted ? showRestrictedTip() : swapLang()"
        >互换语种</el-button>
      </div>
    </div>

    <!-- 操作主卡片 -->
    <div class="operate-card" :class="{ blurred: isRestricted }">
      <!-- 文档上传 -->
      <div class="upload-block">
        ...
        >
          <el-button type="primary" icon="el-icon-upload2" :disabled="loading || isRestricted" @click="isRestricted ? showRestrictedTip() : undefined">上传文档</el-button>
          <template #tip>
            <div class="upload-tip">支持格式：TXT / DOC / DOCX，单次仅可上传 1 个文件</div>
          </template>
        </el-upload>
        <el-button @click="clearFile" class="ml-3" :disabled="loading || isRestricted">清空文件</el-button>
      </div>
      ...
      <div class="btn-block">
        <el-button 
          ...
          @click="isRestricted ? showRestrictedTip() : doTranslate()"
          :loading="loading"
          size="large"
        >
          开始翻译
        </el-button>
        <el-button icon="el-icon-delete" @click="clearAll" size="large" :disabled="loading || isRestricted">清空全部</el-button>
      </div>
    </div>

    <!-- 翻译结果区 -->
    <div class="result-card" :class="{ blurred: isRestricted }">
      <div class="result-head">
        <span class="result-title">翻译结果（左右对照）</span>
        <div class="result-actions">
          <el-button icon="el-icon-document-copy" size="small" @click="isRestricted ? showRestrictedTip() : copyResult()">复制译文</el-button>
          <el-button icon="el-icon-download" size="small" @click="isRestricted ? showRestrictedTip() : exportTxt()">导出TXT</el-button>
        </div>
      </div>
    ...
    <!-- 翻译历史记录 -->
    <div class="history-card" :class="{ blurred: isRestricted }" v-if="historyList.length">
      ...
      <div class="history-list">
        <div 
          ...
          :key="idx"
          @click="isRestricted ? showRestrictedTip() : reuseHistory(item)"
        >
...
/** 是否为受限用户（非管理员） */
const isRestricted = computed(() => !userStore.isAdmin)

const showRestrictedTip = () => {
  ElMessage.warning('翻译功能仅限管理员使用，如需开通请联系管理员')
}
...
const handleFileChange = (file) => {
  if (isRestricted.value) return showRestrictedTip()
