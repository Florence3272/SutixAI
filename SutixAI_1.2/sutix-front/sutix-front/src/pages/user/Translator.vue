...
// 翻译历史记录
const historyList = ref(JSON.parse(localStorage.getItem('transHistory')) || [])
// 本地术语库
const termList = ref([])
...
const handleFileChange = (file) => {
  ...
  if (fileName.endsWith('.txt')) {
    ...
    ElMessage.info('Word 文件请手动复制文本到输入框，浏览器暂不支持直接解析')
  }
}
...
const doTranslate = async () => {
  ...
  try {
    ...
    const requestBody = {
      text: dealText,
      from: transDirection.value === 'zh2ru' ? 'zh' : 'ru',
      to: transDirection.value === 'zh2ru' ? 'ru' : 'zh'
    }
    ...
    resultText.value = data.translatedText || data.result || data.data || ''

    // 3. 存入历史记录（最多保留5条）
    historyList.value.unshift({ text: sourceText.value, res: resultText.value })
    if (historyList.value.length > 5) historyList.value.pop()
    localStorage.setItem('transHistory', JSON.stringify(historyList.value))

    ElMessage.success('翻译完成')
  } catch (err) {
    console.error(err)
    ElMessage.error('翻译失败，请检查 API 配置或网络')
  } finally {
    loading.value = false
  }
}
